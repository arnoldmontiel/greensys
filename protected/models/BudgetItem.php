...
class BudgetItem extends ModelAudit
{
	...
	{
		if($this->description == "Programaci贸n"||$this->description == "Instalaci贸n")
		{
	...
	public function calcTimeBeforeChangeService()
	{
		...
		$modelProgramingHours = BudgetItem::model()->findByAttributes(array('Id_service'=>$this->Id_service, 'Id_budget'=>$this->Id_budget,'version_number'=>$version,'description'=>'Programaci贸n'));
		...
		$modelInstalationHours = BudgetItem::model()->findByAttributes(array('Id_service'=>$this->Id_service, 'Id_budget'=>$this->Id_budget,'description'=>'Instalaci贸n'));
	...
	public function calculateTimes()
	{
		...
		$criteria->compare('t.description','Programaci贸n');
		if(isset($this->Id_service))
		{
		...
		$modelProgramingHours = BudgetItem::model()->find($criteria);
		if(!isset($modelProgramingHours))
		{
			...
			$modelProgramingHours->description = 'Programaci髇';
		...
		$criteria->compare('t.Id_budget',$this->Id_budget);
		$criteria->compare('t.version_number',$version);
		$criteria->compare('t.description','Instalaci髇');
		if(isset($this->Id_service))
		{
			if($this->Id_service != 0)
				$criteria->compare('t.Id_service',$this->Id_service);
		...
		if(!isset($modelInstalationHours))
		{
			...
			$modelInstalationHours->description = 'Instalaci贸n';
