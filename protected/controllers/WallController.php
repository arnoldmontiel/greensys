...
class WallController extends Controller
{
	 ...
	 */
	public $layout='//layouts/tcolumn1';
	...
	{
		$ddlSource = TCustomer::model()->findAll();
	...
	public function actionAjaxRemoveBubble($id)
	{
		...
		try {
			...
			if(isset($model->Id_multimedia)){
				...
				$this->unlinkFile(TMultimedia::model()->findByPk($model->Id_multimedia));
		    	TMultimedia::model()->deleteByPk($model->Id_multimedia);
			}
		    ...
		    {
		    	$this->unlinkAlbumFiles(TMultimedia::model()->findAllByAttributes(array('Id_album'=>$model->Id_album)));
	...
	{	
		if(isset($_POST['Album_Id_album']))
		{
			...
			try {
				...
				TMultimedia::model()->deleteAllByAttributes(array('Id_album'=>$modelAlbum->Id));
