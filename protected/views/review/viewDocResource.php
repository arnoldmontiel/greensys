...
$('#files_container').find('textarea').each(
				function(index, item){
							$(item).change(function(){
								var target = $(this);
								var it = $(item);
								$.get('".TMultimediaController::createUrl('tmultimedia/AjaxAddResourceDescription')."',
 									{
					...
					echo CHtml::openTag('div',array('class'=>'review-update-files-descr'));
						...
						array(
								...
								'class'=>'album-action-remove  album-action-remove-update-file',
                                	'ajax'=> array(
										'type'=>'GET',
										'url'=>TMultimediaController::createUrl('tmultimedia/AjaxRemoveResource',array('IdMultimedia'=>$item->Id)),
										'beforeSend'=>'function(){
